...
public class EntityAIBotArrowAttack extends EntityAIBase {
	...
	public EntityAIBotArrowAttack(EntityLiving par1EntityLiving, float par2,
			int par3, int par4) {
	...
	public void updateTask() {
		double var1 = 100.0D;
		double var3 = this.entityHost.getDistanceSq(this.attackTarget.posX,
				this.attackTarget.boundingBox.minY, this.attackTarget.posZ);
		boolean var5 = this.entityHost.getEntitySenses().canSee(
				this.attackTarget);
		...
		if (var3 <= var1 && this.field_48367_f >= 20) {
			...
			this.entityHost.getNavigator().tryMoveToEntityLiving(
					this.attackTarget, this.field_48370_e);
		}
	...
	private void doRangedAttack() {
		if (this.rangedAttackID == 1) {
			...
			EntitySnowball var9 = new EntitySnowball(this.worldObj,
					this.entityHost);
			double var2 = this.attackTarget.posX - this.entityHost.posX;
			double var4 = this.attackTarget.posY
					+ (double) this.attackTarget.getEyeHeight()
					- 1.100000023841858D - var9.posY;
			double var6 = this.attackTarget.posZ - this.entityHost.posZ;
			float var8 = MathHelper.sqrt_double(var2 * var2 + var6 * var6) * 0.2F;
			var9.setThrowableHeading(var2, var4 + (double) var8, var6, 1.6F,
					12.0F);
			this.worldObj
					.playSoundAtEntity(
							this.entityHost,
